...
@ Objective:  The purpose of this program is to get the student to use the stack to pass 
@ 	parameters to and from thir own defined subroutines/functions in ARM Assembly
@ History:
@	Created 02/18, adding comments when necessary
@
@ Use these commands to assemble, link, run and debug this program:
@    as -o Garnett.o Garnett.s
@    gcc -o Garnett Garnett.o
@    ./Garnett ;echo $?
@    gdb --args ./Garnett 
...
@ case will be intInput. 
   ...
   ldr r10, =hexInput1		@ Load the address of where the first operand will be stored
   ldr r10, [r10]		@ Load the value located at this address into the same register
   ldr r11, =hexInput2 		@ Load the address of where the second operand will be stored
   ldr r11, [r11]		@ Load the value located at this address into the same register
   push {r10, r11}		@ Push these two values onto the stack
...
@*********************
   ...
   bleq switch_and		@ If equal, let switch_and handle
   beq printResults		@ After it is handled, come back and then branch to printResults
   ...
   bleq switch_orr		@ If equal, let switch_orr handle
   beq printResults		@ After it is handled, come back and then branch to printResults
   ...
   bleq switch_xor		@ If equal, let switch_xor handle
   beq printResults		@ After it is handled, come back and then branch to printResults
   ...
   bleq switch_bic		@ If equal, let switch_bic handle
   beq printResults		@ After it is handled, come back and then branch to printResults
...
@*********************
   ...
   bl printf			@ Print results, in '\t = 0x00000000' form
...
@*********************
   ...
   cmpne r1, #'Y'		@ If not 'y', check if it is 'Y'
   beq prompt			@ If it is equal to either 'Y' or 'y', go to beginning, as the user isn't done.
   b myexit			@ The user did not enter 'y' or 'Y'. End program

switch_and:	@ This is responsible for the AND function
   pop {r1, r2}			@ Pop r1 and r2, which were originall r10 and r11 on line 75
   ldr r0, =print_AND		@ Loading in the print format into r0 "[r0] AND [r1]"
   and r4, r1, r2		@ [r4] <- [r1] AND [r2] 
   push {r4}			@ Store r4, the result, on the stack
...
switch_orr:	@ This is responsible for the ORR function
   pop {r1, r2}			@ Pop r1 and r2, which were originall r10 and r11 on line 75
   ldr r0, =print_ORR		@ Loading in the print format into r0 "[r0] ORR [r1]"
   orr r4, r1, r2		@ [r4] <- [r1] ORR [r2] 
   push {r4}			@ Store r4, the result, on the stack
   push {lr}			@ Then, store the link register on the stack.=
   bl printf			@ Call printf
   pop {pc}			@ Pop the link register to the program counter (goes up to if/else)
...
switch_xor:	@ This is responsible for the XOR function
   pop {r1, r2}			@ Pop r1 and r2, which were originall r10 and r11 on line 75
   ldr r0, =print_XOR		@ Loading in the print format into r0 "[r0] XOR [r1]"
...
switch_bic:	@ This is responsible for the BIC function
   pop {r1, r2}			@ Pop r1 and r2, which were originall r10 and r11 on line 75
   ldr r0, =print_BIC		@ Loading in the print format into r0 "[r0] BIC [r1]"
   bic r4, r1, r2		@ [r4] <- [r1] BIC [r2] 
   push {r4}			@ Store r4, the result, on the stack
   push {lr}			@ Then, store the link register on the stack.
   bl printf			@ Call printf
   pop {pc}			@ Pop the link register to the program counter (goes up to if/else)
